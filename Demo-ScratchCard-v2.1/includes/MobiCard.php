...
class MobiCard
{
    ...
    function CardPay($pin_card,$card_serial,$type_card,$_order_id,$client_fullname,$client_mobile,$client_email)
    {
        $params = array(
            ...
            'checksum'		        => MD5(Config::$_MERCHANT_ID.'|'.Config::$_MERCHANT_PASSWORD.'|'.$card_serial),
            'pin_card'				=> $pin_card,
        ...
        if ($result != '' && $status==200){
            ...
            if (count($arr_result) == 13) {
                $kq->error_code	     = $arr_result['error_code'];
                $kq->merchant_id	     = $arr_result['merchant_id'];
                $kq->merchant_account = $arr_result['merchant_account'];
                $kq->pin_card	         = $arr_result['pin_card'];
                $kq->card_serial     = $arr_result['card_serial'];
                $kq->type_card	     = $arr_result['type_card'];
                $kq->order_id		 = $arr_result['ref_code'];
                $kq->client_fullname = $arr_result['client_fullname'];
                $kq->client_email    = $arr_result['client_email'];
                $kq->client_mobile   = $arr_result['client_mobile'];
                $kq->card_amount     = $arr_result['card_amount'];
                $kq->amount			 = $arr_result['transaction_amount'];
                $kq->transaction_id	 = $arr_result['transaction_id'];
